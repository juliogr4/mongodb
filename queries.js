...
db.users.find( {idade: 27, nome: "Julio"} );                      // WHERE idade = 27 AND nome = "Julio"
db.users.find({ $and: [{idade: 27, nome:"Julio"}] });             // WHERE idade = 27 AND nome = "Julio"
...
db.users.find({ nome: "Julio", $or: [{idade: {$in:[27,54]}}] })   // WHERE nome = Julio AND (idade = 27 OR idade = 54)
...
db.users.updateMany(
    { nome: {$in: ["Pai Sidney", "Luna", "bebe"]} },
    { $set: { telefone: 123456, idade: 20 } }
)

// DELETE
db.users.deleteOne({ nome: "Luna" })
db.users.deleteMany({ nome: "Julio" })
